    ...
    queryFn: () =>
      newRequest.get(`/posts?search=${searchInput}&min=${minRef.current.value}&max=${maxRef.current.value}&vehiclemake=${vehicleMakeRef.current.value}&location=${cityRef.current.value}&vehiclemodel=${vehicleModelRef.current.value}`).then((res) => {
        ...
        if (res.status === 204) {
          ...
          // Return data when results are found
          setNoData("");
          return res.data;
        } else {
          // Handle other HTTP error cases
          console.error('Error:', res.status);
          throw new Error('Error fetching data'); // Throw an error to trigger the catch block
        }
      }),    
  });

  let totalSearchedAds;
...
<div className="top-info mt-4 d-flex flex-row justify-content-md-around justify-content-lg-around justify-content-between">
        <div className="foundAds ms-4 ms-md-0 ms-lg-0 me-lg-5" style={{  color:'grey',cursor: 'pointer' }}
        onClick={() => {
          if (filter) {
            clear(); // Call the clear function here
          }
        }}>
        {filter ? (
            <>
      ...
      <div className="foundAds me-4 me-lg-0 me-md-0 ms-lg-5  " style={{  color:'grey' }}>
      {totalSearchedAds ? ("Found Results: " + totalSearchedAds ) : null}
        </div>
      ...
      <div className="container">
        ...
        <div className="error-message" style={{ display:"flex", justifyContent: "center", alignItems: "center", fontSize: "24px", fontWeight: "bold", color: "red", marginTop: "10%", marginBottom: "10%"  }}>
    ...
    :  <div className="no-ads" style={{display:"flex", justifyContent: "center", alignItems: "center", fontSize: "24px", fontWeight: "bold", marginTop: "10%", marginBottom: "10%" }}>No Searched Ads found</div>}
    <div/>
    </div>
    </> )
}
